...
const CFG: GoogleOAuthConfig = {
  ...
  redirectUri: "http://localhost:3010/api/admin/auth/google/callback",
};
...
describe("google-oauth", () => {
  ...
  it("aceita id_token de email autorizado e verificado", () => {
    ...
    const r = validarIdTokenAdmin(info, CFG.clientId, ["contato@example.com"]);
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.email).toBe("contato@example.com");
  });
  ...
  it("rejeita email não autorizado", () => {
    ...
    const r = validarIdTokenAdmin(info, CFG.clientId, ["admin@example.com"]);

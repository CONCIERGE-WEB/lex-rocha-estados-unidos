...
export function PricingSection() {
  ...
  return (
    <section id="planos" className="scroll-mt-20 section-dark py-16 md:py-24">
      <div className="mx-auto max-w-7xl px-4 md:px-6">
        <div className="max-w-2xl">
          <p className="font-mono text-xs font-bold uppercase tracking-[0.2em] text-onDarkMuted">
            {P.eyebrow}
          </p>
          <h2 className="mt-3 font-display text-3xl font-bold text-onDark md:text-4xl">{P.title}</h2>
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-3">
          {PLANOS.map((plano) => {
            ...
            return (
              <article
                key={plano.id}
                className={`relative flex flex-col rounded-xl border p-7 transition md:p-8 ${
                  destaque
                    ? "z-10 scale-[1.02] border-verify/50 bg-gradient-to-b from-trustDeep to-ink shadow-lift lg:-my-2"
                    : "border-onDark/20 bg-folio/5 hover:border-onDark/35"
                }`}
              >
                {destaque ? (
                ...
                <ul className="mt-6 flex-1 space-y-2.5 border-t border-onDark/15 pt-6 text-sm font-medium text-onDark">
        ...
        <p className="mt-12 max-w-3xl border-t border-onDark/15 pt-8 text-sm font-medium leading-relaxed text-onDarkMuted">
